...
export function Header() {
  ...
  return (
    <header className="border-b border-border-subtle bg-bg-secondary/70 backdrop-blur">
      <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col md:flex-row items-start gap-2 md:items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-gradient-to-br text-white from-primary-500 to-primary-700 shadow-glow grid place-items-center font-mono text-sm font-bold">
            CS
          </div>
          <div>
        ...
        <div className="flex items-center gap-4 text-xs text-text-secondary font-mono">
          <span>{mounted ? count : 0} tasks</span>
          <span className="text-text-muted">·</span>
          <span>
            snapshot {mounted ? history.currentIndex + 1 : 0}/{mounted ? history.snapshots.length : 0}
          </span>
        </div>
      </div>
    </header>
